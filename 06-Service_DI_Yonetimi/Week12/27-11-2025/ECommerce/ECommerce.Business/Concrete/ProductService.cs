...
public class ProductService : IProductService
{
        ...
        FilterDto? filterDto)
    {
        try
        {
            ...
            if (filterDto is not null && filterDto.IsDeleted.HasValue)
            {
                predicate = predicate.And(x => x.IsDeleted == filterDto.IsDeleted);
            }

            if (filterDto is not null && filterDto.CategoryId.HasValue)
            {
                predicate = predicate.And(x => x.ProductCategories.Any(pc => pc.CategoryId == filterDto.CategoryId.Value));
            }

            var includeList = new List<Func<IQueryable<Product>, IQueryable<Product>>>();
            if (filterDto is not null && filterDto.IncludeCategories.HasValue)
            {
            ...
            var products = await _productRepository.GetAllAsync(
                predicate: predicate,
                showIsDeleted: filterDto?.IsDeleted ?? false,
                orderBy: orderBy!,
                asExpanded: true,
    ...
    public async Task<ResponseDto<IEnumerable<ProductDto>>> GetLowStockAsync(int threshold)
    {
        ...
        {
            var products = await _productRepository.GetAllAsync(
                predicate: x => x.StockQuantity < threshold, orderBy: x => x.OrderByDescending(y => y.StockQuantity)
            );
            if (!products.Any())
            {
                return ResponseDto<IEnumerable<ProductDto>>.Fail("Hiç ürün bulunamadı!", StatusCodes.Status404NotFound);
    ...
    public async Task<ResponseDto<NoContent>> UpdateAsync(int id, ProductUpdateDto productUpdateDto)
    {
        try
        {
            ...
            foreach (var categoryId in productUpdateDto.CategoryIds)
            {
                ...
                if (!categoryExists)
                {
                    return ResponseDto<NoContent>.Fail($"{categoryId} id'li kategori veri tabanında bulunamadığı için kayıt gerçekleştirilemedi!", StatusCodes.Status400BadRequest);
                }
            }
